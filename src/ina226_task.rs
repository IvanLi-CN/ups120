...
pub async fn ina226_task(
    ...
    if (ina226.callibrate(0.01, 10.0).await).is_err() {
    ...
    loop {
        ...
        let current_a = ina226.current_amps().await.unwrap_or(None).unwrap_or(0.0);
        let power_w = ina226.power_watts().await.unwrap_or(None).unwrap_or(0.0);
