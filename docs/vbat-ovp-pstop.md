...
### 2.5 项目实际参数（按网表）
- 电池侧外部分压（来自网表 /Users/ivan/Downloads/Netlist_PCB1_2_2025-10-27.enet）：
  - R35=140 kΩ（PACK+ → 分压节点 $1N20113）
  - R37=10 kΩ（分压节点 $1N20113 → GND）
  - U4=SC8815 引脚：pin16 连到分压节点（等效 VBATS 外部分压输入）；pin14=FB 为 VBUS 反馈，非电池限压输入。
- 充电限制电压（SC8815 外部参考 1.20 V）：
  - V_charge_limit(bat) = 1.20 × (1 + 140k/10k) = 1.20 × 15 = 18.00 V
- 过压触发（本方案采用 TLV431 1.24 V 参考 + LMV331 比较，同一分压节点）：
  - V_trip(bat) = 1.24 × (1 + 140k/10k) = 1.24 × 15 = 18.60 V
  - 等价为相对限压 +3.33% 的“明显过压”触发（1.24/1.20 ≈ 1.0333）。
- 容差与建议：
  - 参考精度主导触发误差。若用 TLV431A（±1%）：V_trip ≈ 18.60 V ±1%（≈ 18.41–18.79 V）；用 B 级（±0.5%）可收敛到 ≈ 18.51–18.69 V。
  - 比较器输入失调（数 mV）对 1.24 V 的相对误差一般 <0.5%，可与参考误差合并评估。
  - 分压电阻容差对“相对限压”的裕量影响很小（限压与保护共用同一分压比，比例误差基本抵消）；仍建议选用 1%（或 0.5%）电阻，保证绝对值一致性与温漂。

### 2.6 主要商品链接（同店铺）
- TLV431（1.24 V 基准，SOT‑23）：
  - TLV431AIDBZR（±1%）：https://item.taobao.com/item.htm?id=675960221171
  - TLV431BQDBZR（±0.5%）：https://item.taobao.com/item.htm?id=862800443494
- LMV331（单通道比较器，开漏输出，SOT‑23‑5）：
  - LMV331IDBVR：https://item.taobao.com/item.htm?id=674953169411
- 逻辑门（整形与一票否决汇总）
  - SN74LVC1G14DBVR（施密特反相）：https://item.taobao.com/item.htm?id=673726619913
  - SN74LVC1G32DBVR（OR 门）：https://item.taobao.com/item.htm?id=672658924086
