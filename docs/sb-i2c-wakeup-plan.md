...
4) 对外协议（I2C1 从机）：
   ...
   - `CHG_CONFIG`（0x31）提供充电策略配置（bit0=auto，bit1=manual_enable，bit[3:2]=速率档）。
...
CHG_CONFIG（0x31, RW）的说明：

- `AUTO` 置 1 时启用固件自主策略（寄存器返回当前充电请求状态）；
- `AUTO` 置 0 后 `MANUAL_ENABLE` 位才可写，用于显式允许/禁止充电（仍受安全策略约束）；
- bit[3:2] 承载充电速率（0=慢充，1≈0.8A，2≈1.2A，3≈1.6A），写入时需与模式位同帧更新。
