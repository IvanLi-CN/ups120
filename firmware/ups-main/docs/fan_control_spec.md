...
1. 温度 → 占空比策略（占空比指 **GPIO40 实际 PWM%**）：
   - **VIN 存在**（默认 `VIN_PRESENT_DEFAULT=true`）：
     - `<35 °C`：关闭风扇（`FAN_EN=Low`）。
     - `35–45 °C`：固定 96%，并清空 PI 状态。
     - `>45 °C`：进入压温区，目标温度 45 °C，占空比允许范围 96%~0%，调节一直持续到温度回落至 `<35 °C` 再停转。
   - **VIN 缺失**：
     - `<40 °C`：关闭风扇。
     - `≥40 °C`：直接进入压温区（目标同样 45 °C），占空比范围保持 96%~0%。
   - PI 参数（`PI_KP`, `PI_KI`）可按硬件表现微调；保持 `96%` 为最小有效占空比。
2. 温度滞回：±3 °C（防抖）；占空比限斜率：每周期最大 ±5%（在“风量”域内运算，然后换算成实际占空比）。
3. 风扇开启流程：占空比从 0 → 非 0 时先 `FAN_EN=High` 再设置 PWM；关闭则维持 `GPIO40=100%` 同时 `FAN_EN=Low`，确保 LDO 输出完全关闭。
4. 保护逻辑：
...
- `ATTR`：当前量程索引；
- `DELTA`：eFuse 校准值（°C）；
- `DUTY`：实际 PWM 占空比（`GPIO40`），风扇关闭时记为 `0`；
- `MODE`：`NORMAL` / `SAFE` / `OVERTEMP`；
- `VOUT`：根据线性模型推算的目标电压（用于调试，风扇关闭时输出 `0`）。
